...
final class MapRegisterer implements Listener {
	...
	public function chatCommand(PlayerChatEvent $event) : void {
		...
		if (strtolower($player->getName()) == strtolower($this->player->getName())) {
			switch (strtolower($args[0])) {
				...
				case "setcages":
					$player->getInventory()->addItem(VanillaItems::BLAZE_ROD()->setCustomName("§r§aSet cages\n§7Break a block."));
	...
	public function onBreak(BlockBreakEvent $event) : void {
		...
		if (strtolower($player->getName()) == strtolower($this->player->getName())) {
			...
			if ($itemId == ItemTypeIds::BLAZE_ROD && $itemName === "§r§aSet platform\n§7Break a corner.") {
			...
			if ($itemId == ItemTypeIds::BLAZE_ROD && $itemName === "§r§aSet cages\n§7Break a block.") {
